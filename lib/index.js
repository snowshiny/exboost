...
Exboost.start = function (options) {
    ...
    let { enableCookie, jwt, port: dfPort, enableLog, useStatic, cors  } = options ;
    enableLog && app.use(Morgan(LogFormat));
    cors && app.use((req, res,next)=>{
        let acr = req.header('Access-Control-Request-Headers');
        res.setHeader('Access-Control-Allow-Origin', '*');
        acr && res.setHeader('Access-Control-Allow-Headers', acr);
